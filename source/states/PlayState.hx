...
{
	public static var textFont:String = 'DigitalDisco.ttf';
	...
	override public function create()
	{
		...
		if (barcodeFont && textFont != 'LibreBarcode128Text-Regular.ttf')
			textFont = 'LibreBarcode128Text-Regular.ttf';
		...
		|| ClientPrefs.getGameplaySetting('practice') && !practiceMode))
		{
			...
			if (ClientPrefs.data.ghostTapping && songName == 'spitting-facts')
				hahaText += 'Ghost Tapping has been turned off for this song!\n';

			if (ClientPrefs.getGameplaySetting('botplay') && !cpuControlled || ClientPrefs.getGameplaySetting('practice') && !practiceMode)
			{
				var extraHahaText:String = '';

				if (ClientPrefs.getGameplaySetting('botplay') && !cpuControlled)
					extraHahaText += 'Botplay';
				else if (ClientPrefs.getGameplaySetting('practice') && !practiceMode)
					extraHahaText += 'Practice Mode';

				hahaText += 'Nice try, ' + extraHahaText + ' has been turned off for this song!\n';
			}
	...
	public function reloadHealthBarColors() {
		healthBar.setColors(FlxColor.fromRGB(iconP2.healthFile.colors[0], iconP2.healthFile.colors[1], iconP2.healthFile.colors[2]),
			FlxColor.fromRGB(iconP1.healthFile.colors[0], iconP1.healthFile.colors[1], iconP1.healthFile.colors[2]));

		healthOppTxt.color = healthBar.leftBar.color;
		healthPlayerTxt.color = healthBar.rightBar.color;
	}
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true)
	{
		...
		if (foundFile)
		{
			...
			if (playOnLoad)
			{
				...
				if (!loop)
				{
					skipBG = new FlxSprite(0, FlxG.height).makeGraphic(1, 1, FlxColor.BLACK);
					skipBG.alpha = 0.6;
					skipBG.camera = camOther;
					add(skipBG);
					...
					skipBG.setGraphicSize(skipTxt.width + skipTxt.size / 5, skipTxt.size * 1.5);
					skipBG.updateHitbox();

					skipBG.y -= skipBG.height;
					skipTxt.setPosition(skipTxt.size / 5, skipBG.y + skipTxt.height / 5);
				}
			}
			return cutscene;
		}
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		if (paused)
		{
			...
			FlxTween.globalManager.forEach(function(twn:FlxTween) if(!twn.finished && twn != SystemUtil.windowOpacityTween) twn.active = false);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				if(dunceNote.noteType == 'Second Character Note')
					dunceNote.alpha -= 0.5;
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		{
			var missAnim:String = singAnimations[Std.int(Math.abs(Math.min(singAnimations.length-1, direction)))] + 'miss';

			var suffix:String = '';
			if(note != null && char.animOffsets.exists(missAnim + note.animSuffix)) suffix = note.animSuffix;

			var animToPlay:String = missAnim + suffix;
			char.playAnim(animToPlay, true);

			if(char != gf && lastCombo > 5 && gf != null && gf.animation.exists('sad'))
			{
				gf.playAnim('sad');
				gf.specialAnim = true;
			}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!cpuControlled && note.lightStrumline)
		{
			var spr = playerStrums.members[note.noteData];
			if(spr != null) spr.playAnim('confirm', true);
		}
		else if(note.lightStrumline) strumPlayAnim(false, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate);
		vocals.volume = 1;
	...
	public function spawnNoteSplashOnNote(note:Note, player:Bool = true) {
		if(note != null)
		{
