...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var defaultList:Array<Array<String>> = [
			...
			['MiddestReal',			'mid',				'Charter',													 'https://twitter.com/MiddestReal',		'FF0000'],
			[''],
			['SNC Socials!'],
			...
			["MAJigsaw77",          "face",             ".MP4 Video Loader Library (hxvlc)",                         "https://twitter.com/MAJigsaw77",      "A1A1A1"],
		...
		for (i in 0...creditsStuff.length)
		{
			...
			if(isSelectable) {
				...
				if(Paths.fileExists('images/$str.xml', TEXT))
				{
					var iconName:String = str.replace('credits/', '');
					icon.frames = Paths.getSparrowAtlas(str);
					icon.animation.addByPrefix('normal', iconName, 24, true);
